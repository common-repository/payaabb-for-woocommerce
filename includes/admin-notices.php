...
class WC_Payaabb_Admin_Notices
{
    public $notices = [];
    ...
    public function admin_notices()
    {
        ...
        wp_nonce_url(
            add_query_arg("wc-payaabb-hide-notice", $notice_key),
            "WC_Payaabb_hide_notices_nonce",
            "_WC_Payaabb_notice_nonce"
        )
    ...
    {
        $public_plugin_url =
            "https://wordpress.org/plugins/payaabb-for-woocommerce/";
        $last_check_datetime = get_option(
            "wc_payaabb_latest_datetime_check_public_version_plugin"
        );
    ...
    {
        $show_keys_notice = get_option("wc_payaabb_show_keys_notice");
        $show_ssl_notice = get_option("wc_payaabb_show_ssl_notice");
        $show_phpver_notice = get_option("wc_payaabb_show_phpver_notice");
        $show_wcver_notice = get_option("wc_payaabb_show_wcver_notice");
        $show_curl_notice = get_option("wc_payaabb_show_curl_notice");
        $show_curl_notice = get_option("wc_payaabb_show_curl_notice");
        $options = get_option("woocommerce_payaabb_settings");
        ...
        if (isset($options["enabled"]) && "yes" === $options["enabled"]) {
            if (empty($show_phpver_notice)) {
                if (
                    version_compare(phpversion(), WC_PAYAABB_MIN_PHP_VER, "<")
                ) {
                    $this->add_admin_notice(
                        "phpver",
                        "error",
                        sprintf(
                            ...
                            esc_html(WC_PAYAABB_MIN_PHP_VER),
                            esc_html(phpversion())
                        ),
                        true
                    );
            ...
            if (empty($show_wcver_notice)) {
                if (version_compare(WC_VERSION, WC_PAYAABB_MIN_WC_VER, "<")) {
                    $this->add_admin_notice(
                        "wcver",
                        "notice notice-warning",
                        sprintf(
                            ...
                            esc_html(WC_PAYAABB_MIN_WC_VER),
                            esc_html(WC_VERSION)
                        ),
                        true
                    );
    ...
    {
        if (
            isset($_GET["wc-payaabb-hide-notice"]) &&
            isset($_GET["_WC_Payaabb_notice_nonce"])
        ) {
            if (
                !wp_verify_nonce(
                    sanitize_text_field(
                        wp_unslash($_GET["_WC_Payaabb_notice_nonce"])
                    ),
                    "WC_Payaabb_hide_notices_nonce"
                )
            ) {
            ...
            if (!current_user_can("manage_woocommerce")) {
                wp_die(esc_html__("Access denied", "payaabb-for-woocommerce"));
            }

            $notice = wc_clean($_GET["wc-payaabb-hide-notice"]);

            switch ($notice) {
                case "phpver":
                    update_option("WC_Payaabb_show_phpver_notice", "no");
                    break;
                case "wcver":
                    update_option("WC_Payaabb_show_wcver_notice", "no");
                    break;
                case "curl":
                    update_option("WC_Payaabb_show_curl_notice", "no");
                    break;
                case "keys":
                    update_option("WC_Payaabb_show_keys_notice", "no");
                    break;
                case "ssl":
                    update_option("WC_Payaabb_show_ssl_notice", "no");
                    break;
            }
        }
    }
